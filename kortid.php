<style>
th {
  display: table-cell;
  vertical-align: inherit;
  font-weight: bold;
  text-align: center;
}
</style>
<h1>Körtider</h1>

<br><br><br>

<center>
    <table width="10%" border="0" cellspacing="0" cellpadding="5" align="center">
      <tbody>
        <tr>
           <td width="10%" style="text-align: center; padding-right: 20px;"><select id="myInput5" onchange="searchTable()" placeholder="Role Category..">
              <option disabled selected value>Från</option>
			  
<option value="Alingsås">Alingsås</option>
<option value="Anneberg">Anneberg</option>
<option value="Aspedalen">Aspedalen</option>
<option value="Aspen">Aspen</option>
<option value="Assberg">Assberg</option>
<option value="Bankeryd">Bankeryd</option>
<option value="Berghem">Berghem</option>
<option value="Björketorp">Björketorp</option>
<option value="Blomberg">Blomberg</option>
<option value="Bohus">Bohus</option>
<option value="Bollebygd">Bollebygd</option>
<option value="Borgstena">Borgstena</option>
<option value="Borås">Borås</option>
<option value="Derome">Derome</option>
<option value="Dingle">Dingle</option>
<option value="Ed">Ed</option>
<option value="Falköping">Falköping</option>
<option value="Filsbäck">Filsbäck</option>
<option value="Floby">Floby</option>
<option value="Floda">Floda</option>
<option value="Forserum">Forserum</option>
<option value="Forshem">Forshem</option>
<option value="Framnäs City">Framnäs City</option>
<option value="Fristad">Fristad</option>
<option value="Fritsla">Fritsla</option>
<option value="Gamlestaden">Gamlestaden</option>
<option value="Grästorp">Grästorp</option>
<option value="Gårdsjö">Gårdsjö</option>
<option value="Göteborg">Göteborg</option>
<option value="Habo">Habo</option>
<option value="Halden">Halden</option>
<option value="Hallsberg">Hallsberg</option>
<option value="Hasslerör">Hasslerör</option>
<option value="Hede">Hede</option>
<option value="Herrljunga">Herrljunga</option>
<option value="Hestra">Hestra</option>
<option value="Hindås">Hindås</option>
<option value="Horred">Horred</option>
<option value="Hova">Hova</option>
<option value="Huskvarna">Huskvarna</option>
<option value="Håkantorp">Håkantorp</option>
<option value="Hällekis">Hällekis</option>
<option value="Hällevadsholm">Hällevadsholm</option>
<option value="Jonsered">Jonsered</option>
<option value="Järpås">Järpås</option>
<option value="Jönköping">Jönköping</option>
<option value="Kinna">Kinna</option>
<option value="Knalleland">Knalleland</option>
<option value="Kode">Kode</option>
<option value="Kumla">Kumla</option>
<option value="Kungsbacka">Kungsbacka</option>
<option value="Kållered">Kållered</option>
<option value="Källby">Källby</option>
<option value="Laxå">Laxå</option>
<option value="Lerum">Lerum</option>
<option value="Lidköping">Lidköping</option>
<option value="Limmared">Limmared</option>
<option value="Lindome">Lindome</option>
<option value="Liseberg">Liseberg</option>
<option value="Ljung">Ljung</option>
<option value="Ljungskile">Ljungskile</option>
<option value="Lovene">Lovene</option>
<option value="Lugnås">Lugnås</option>
<option value="Lyrestad">Lyrestad</option>
<option value="Lödöse Södra">Lödöse Södra</option>
<option value="Mariestad">Mariestad</option>
<option value="Mellerud">Mellerud</option>
<option value="Mollaryd">Mollaryd</option>
<option value="Mullsjö">Mullsjö</option>
<option value="Munkedal">Munkedal</option>
<option value="Mölndal">Mölndal</option>
<option value="Mölnlycke">Mölnlycke</option>
<option value="Nol">Nol</option>
<option value="Norsesund">Norsesund</option>
<option value="Nässjö">Nässjö</option>
<option value="Nödinge">Nödinge</option>
<option value="Partille">Partille</option>
<option value="Rabbalshede">Rabbalshede</option>
<option value="Råbäck">Råbäck</option>
<option value="Rävlanda">Rävlanda</option>
<option value="Sandared">Sandared</option>
<option value="Sandhem">Sandhem</option>
<option value="Skee">Skee</option>
<option value="Skene">Skene</option>
<option value="Skövde">Skövde</option>
<option value="Stenkullen">Stenkullen</option>
<option value="Stenstorp">Stenstorp</option>
<option value="Stenungsund">Stenungsund</option>
<option value="Stora höga">Stora höga</option>
<option value="Stora Levene">Stora Levene</option>
<option value="Strömstad">Strömstad</option>
<option value="Surte">Surte</option>
<option value="Svenshögen">Svenshögen</option>
<option value="Säffle">Säffle</option>
<option value="Sävenäs">Sävenäs</option>
<option value="Tanum">Tanum</option>
<option value="Tenhult">Tenhult</option>
<option value="Tofta">Tofta</option>
<option value="Torpåkra">Torpåkra</option>
<option value="Torved">Torved</option>
<option value="Trollhättan">Trollhättan</option>
<option value="Trolmen">Trolmen</option>
<option value="Töreboda">Töreboda</option>
<option value="Uddevalla C">Uddevalla C</option>
<option value="Uddevalla Östra">Uddevalla Östra</option>
<option value="Vara">Vara</option>
<option value="Varberg">Varberg</option>
<option value="Vargön">Vargön</option>
<option value="Veddige">Veddige</option>
<option value="Vedum">Vedum</option>
<option value="Viskafors">Viskafors</option>
<option value="Vårgårda">Vårgårda</option>
<option value="Vänersborg">Vänersborg</option>
<option value="Västra Bodarne">Västra Bodarne</option>
<option value="Ytterby">Ytterby</option>
<option value="Åb">Åb</option>
<option value="Åmål">Åmål</option>
<option value="Åsa">Åsa</option>
<option value="Älvängen">Älvängen</option>
<option value="Äskekärr">Äskekärr</option>
<option value="Örebro">Örebro</option>
<option value="Örebro Södra">Örebro Södra</option>
<option value="Österäng">Österäng</option>
<option value="Överby">Överby</option>
<option value="Öxnered">Öxnered</option>

              </select>
            </td>
           <td width="15%" style="text-align: center; padding-left: 2px;"><select id="myInput6" onchange="searchTable()" placeholder="Role Category..">
              <option disabled selected value>Till</option>
<option value="Alingsås">Alingsås</option>
<option value="Anneberg">Anneberg</option>
<option value="Aspedalen">Aspedalen</option>
<option value="Aspen">Aspen</option>
<option value="Assberg">Assberg</option>
<option value="Bankeryd">Bankeryd</option>
<option value="Berghem">Berghem</option>
<option value="Björketorp">Björketorp</option>
<option value="Blomberg">Blomberg</option>
<option value="Bohus">Bohus</option>
<option value="Bollebygd">Bollebygd</option>
<option value="Borgstena">Borgstena</option>
<option value="Borås">Borås</option>
<option value="Derome">Derome</option>
<option value="Dingle">Dingle</option>
<option value="Ed">Ed</option>
<option value="Falköping">Falköping</option>
<option value="Filsbäck">Filsbäck</option>
<option value="Floby">Floby</option>
<option value="Floda">Floda</option>
<option value="Forserum">Forserum</option>
<option value="Forshem">Forshem</option>
<option value="Framnäs City">Framnäs City</option>
<option value="Fristad">Fristad</option>
<option value="Fritsla">Fritsla</option>
<option value="Gamlestaden">Gamlestaden</option>
<option value="Grästorp">Grästorp</option>
<option value="Gårdsjö">Gårdsjö</option>
<option value="Göteborg">Göteborg</option>
<option value="Habo">Habo</option>
<option value="Halden">Halden</option>
<option value="Hallsberg">Hallsberg</option>
<option value="Hasslerör">Hasslerör</option>
<option value="Hede">Hede</option>
<option value="Herrljunga">Herrljunga</option>
<option value="Hestra">Hestra</option>
<option value="Hindås">Hindås</option>
<option value="Horred">Horred</option>
<option value="Hova">Hova</option>
<option value="Huskvarna">Huskvarna</option>
<option value="Håkantorp">Håkantorp</option>
<option value="Hällekis">Hällekis</option>
<option value="Hällevadsholm">Hällevadsholm</option>
<option value="Jonsered">Jonsered</option>
<option value="Järpås">Järpås</option>
<option value="Jönköping">Jönköping</option>
<option value="Kinna">Kinna</option>
<option value="Knalleland">Knalleland</option>
<option value="Kode">Kode</option>
<option value="Kumla">Kumla</option>
<option value="Kungsbacka">Kungsbacka</option>
<option value="Kållered">Kållered</option>
<option value="Källby">Källby</option>
<option value="Laxå">Laxå</option>
<option value="Lerum">Lerum</option>
<option value="Lidköping">Lidköping</option>
<option value="Limmared">Limmared</option>
<option value="Lindome">Lindome</option>
<option value="Liseberg">Liseberg</option>
<option value="Ljung">Ljung</option>
<option value="Ljungskile">Ljungskile</option>
<option value="Lovene">Lovene</option>
<option value="Lugnås">Lugnås</option>
<option value="Lyrestad">Lyrestad</option>
<option value="Lödöse Södra">Lödöse Södra</option>
<option value="Mariestad">Mariestad</option>
<option value="Mellerud">Mellerud</option>
<option value="Mollaryd">Mollaryd</option>
<option value="Mullsjö">Mullsjö</option>
<option value="Munkedal">Munkedal</option>
<option value="Mölndal">Mölndal</option>
<option value="Mölnlycke">Mölnlycke</option>
<option value="Nol">Nol</option>
<option value="Norsesund">Norsesund</option>
<option value="Nässjö">Nässjö</option>
<option value="Nödinge">Nödinge</option>
<option value="Partille">Partille</option>
<option value="Rabbalshede">Rabbalshede</option>
<option value="Råbäck">Råbäck</option>
<option value="Rävlanda">Rävlanda</option>
<option value="Sandared">Sandared</option>
<option value="Sandhem">Sandhem</option>
<option value="Skee">Skee</option>
<option value="Skene">Skene</option>
<option value="Skövde">Skövde</option>
<option value="Stenkullen">Stenkullen</option>
<option value="Stenstorp">Stenstorp</option>
<option value="Stenungsund">Stenungsund</option>
<option value="Stora höga">Stora höga</option>
<option value="Stora Levene">Stora Levene</option>
<option value="Strömstad">Strömstad</option>
<option value="Surte">Surte</option>
<option value="Svenshögen">Svenshögen</option>
<option value="Säffle">Säffle</option>
<option value="Sävenäs">Sävenäs</option>
<option value="Tanum">Tanum</option>
<option value="Tenhult">Tenhult</option>
<option value="Tofta">Tofta</option>
<option value="Torpåkra">Torpåkra</option>
<option value="Torved">Torved</option>
<option value="Trollhättan">Trollhättan</option>
<option value="Trolmen">Trolmen</option>
<option value="Töreboda">Töreboda</option>
<option value="Uddevalla C">Uddevalla C</option>
<option value="Uddevalla Östra">Uddevalla Östra</option>
<option value="Vara">Vara</option>
<option value="Varberg">Varberg</option>
<option value="Vargön">Vargön</option>
<option value="Veddige">Veddige</option>
<option value="Vedum">Vedum</option>
<option value="Viskafors">Viskafors</option>
<option value="Vårgårda">Vårgårda</option>
<option value="Vänersborg">Vänersborg</option>
<option value="Västra Bodarne">Västra Bodarne</option>
<option value="Ytterby">Ytterby</option>
<option value="Åb">Åb</option>
<option value="Åmål">Åmål</option>
<option value="Åsa">Åsa</option>
<option value="Älvängen">Älvängen</option>
<option value="Äskekärr">Äskekärr</option>
<option value="Örebro">Örebro</option>
<option value="Örebro Södra">Örebro Södra</option>
<option value="Österäng">Österäng</option>
<option value="Överby">Överby</option>
<option value="Öxnered">Öxnered</option>
</center>
              </select>
          </td>
        </tr>
      </tbody>
</table>
<table id="table" >


    </table>
	<script  src="./bg/kor3.js"></script>